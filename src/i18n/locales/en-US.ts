export const enUS = {
  app: {
    title: 'Headscale Web',
  },
  nav: {
    dashboard: 'Dashboard',
    users: 'Users',
    nodes: 'Nodes',
    routes: 'Routes',
    preAuthKeys: 'PreAuth Keys',
    menu: 'Menu',
    primary: 'Primary navigation',
    settings: 'Settings',
  },
  connection: {
    title: 'Connect to Headscale',
    description: 'Manage users, nodes, routes, and access keys from one secure workspace.',
    securityTitle: 'Your key stays in this browser',
    securityDescription: 'Credentials are sent directly to your Headscale instance.',
    urlLabel: 'Headscale URL',
    urlPlaceholder: 'https://headscale.example.com',
    apiKeyLabel: 'API Key',
    apiKeyPlaceholder: 'Enter your API key',
    showApiKey: 'Show API key',
    hideApiKey: 'Hide API key',
    persistenceTitle: 'Credential storage',
    persistenceSession: 'This session',
    persistenceSessionHint: 'Cleared when the browser closes',
    persistenceLocal: 'Remember on this device',
    persistenceLocalHint: 'Available after you return',
    localRisk:
      'The API key will be stored in localStorage and can be read by other scripts on this origin.',
    localRiskConfirm: 'I understand the risk of long-term storage',
    connect: 'Connect',
    connecting: 'Connecting',
    checksTitle: 'Connection checks',
    checksHint: 'Validated when you connect',
    steps: {
      network: 'Network',
      version: 'Version',
      database: 'Database',
      authorization: 'Authorization',
    },
    states: {
      idle: 'Waiting',
      running: 'Checking',
      ok: 'Ready',
      fail: 'Failed',
    },
    errors: {
      empty: 'Enter a Headscale URL.',
      invalid: 'Enter a valid URL.',
      unsupportedProtocol: 'Use http or https.',
      credentialsNotAllowed: 'Do not include credentials in the URL.',
      network: 'Could not reach Headscale.',
      timeout: 'The request timed out.',
      cors: 'The browser blocked the request. Check CORS on the Headscale origin.',
      unsupportedVersion: 'This UI only supports Headscale 0.29.x.',
      unauthorized: 'The API key was rejected.',
      unknown: 'Something went wrong while connecting.',
    },
  },
  shell: {
    disconnect: 'Disconnect',
    consoleLabel: 'Admin console',
    version: 'Version',
    databaseConnected: 'Database connected',
    databaseDisconnected: 'Database disconnected',
    refresh: 'Refresh',
    language: 'Language',
    theme: 'Theme',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
  },
  common: {
    loading: 'Loading',
    empty: 'No data',
    error: 'Could not load data',
    unavailable: 'Unavailable',
    retry: 'Retry',
    search: 'Search',
    filter: 'Filter',
    online: 'Online',
    offline: 'Offline',
    yes: 'Yes',
    no: 'No',
    details: 'Details',
    close: 'Close',
    create: 'Create',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    rename: 'Rename',
    approve: 'Approve',
    revoke: 'Revoke',
    expire: 'Expire',
    copy: 'Copy',
    confirm: 'Confirm',
    success: 'Saved',
    failed: 'The request failed',
  },
  users: {
    title: 'Users',
    description: 'Create and manage identities that own Headscale nodes.',
    createTitle: 'Create user',
    deleteTitle: 'Delete user',
    confirmDelete: 'Confirm delete',
    dangerZone: 'Danger zone',
    deleteHint: 'This user currently owns {count} nodes.',
    deleteMessage:
      'Delete {name}? This user currently owns {count} nodes and this action cannot be undone.',
    name: 'Name',
    displayName: 'Display name',
    email: 'Email',
    provider: 'Provider',
    createdAt: 'Created',
    allProviders: 'All providers',
    confirmName: 'Type the user name to confirm',
    relatedNodes: 'Related nodes',
    nodeCount: 'Related node count',
    viewNodes: 'View {count} related nodes for {name}',
  },
  nodes: {
    title: 'Nodes',
    description: 'Monitor connected devices, ownership, addressing, tags, and expiry.',
    allStatuses: 'All statuses',
    owner: 'Owner',
    allUsers: 'All users',
    overview: 'Overview',
    keys: 'Masked keys',
    createdAt: 'Created',
    dangerZone: 'Danger zone',
    tagsPlaceholder: 'tag:server, tag:internal',
    expireTitle: 'Expire node now',
    expireMessage: 'Expire {name} immediately? The node will need to authenticate again.',
    deleteTitle: 'Delete node',
    deleteMessage: 'Delete {name}? This action cannot be undone.',
    confirmDelete: 'Confirm delete',
    name: 'Name',
    user: 'User',
    ip: 'IP addresses',
    tags: 'Tags',
    routes: 'Routes',
    registerMethod: 'Register method',
    lastSeen: 'Last seen',
    expiry: 'Expiry',
    status: 'Status',
    machineKey: 'Machine key',
    nodeKey: 'Node key',
    discoKey: 'Disco key',
    expireNow: 'Expire now',
    disableExpiry: 'Disable expiry',
    setTags: 'Set tags',
    confirmName: 'Type the node name to confirm',
  },
  routes: {
    title: 'Routes',
    description: 'Review advertised networks and control which routes are approved and serving.',
    revokeTitle: 'Revoke route',
    revokeMessage: 'Revoke {prefix} from {node}? Traffic using this route may be interrupted.',
    prefix: 'Prefix',
    node: 'Node',
    advertised: 'Advertised',
    approved: 'Approved',
    serving: 'Serving',
    filterPending: 'Pending',
    filterApproved: 'Approved',
    filterExit: 'Exit node',
    filterSubnet: 'Subnet',
    filterAll: 'All',
    approveAll: 'Approve advertised',
    revokeAll: 'Revoke approved',
  },
  preAuthKeys: {
    title: 'PreAuth Keys',
    description: 'Issue and manage enrollment credentials for new nodes.',
    createTitle: 'Create PreAuth Key',
    createdTitle: 'PreAuth Key created',
    properties: 'Properties',
    defaultExpiration: 'The key will expire in 90 days.',
    expireTitle: 'Expire PreAuth Key',
    expireMessage: 'Expire key {id}? It will no longer enroll nodes.',
    deleteTitle: 'Delete PreAuth Key',
    deleteMessage: 'Delete key {id} owned by {user}? This action cannot be undone.',
    deleteExpired: 'Delete expired ({count})',
    deleteExpiredAction: 'Delete expired',
    deleteExpiredTitle: 'Delete expired PreAuth Keys',
    deleteExpiredMessage: 'Delete all {count} expired PreAuth Keys? This action cannot be undone.',
    deleteExpiredPartial: 'Deleted {deleted} expired keys; {failed} failed.',
    deleteUsed: 'Delete used ({count})',
    deleteUsedAction: 'Delete used',
    deleteUsedTitle: 'Delete used PreAuth Keys',
    deleteUsedMessage: 'Delete all {count} used PreAuth Keys? This action cannot be undone.',
    deleteUsedPartial: 'Deleted {deleted} used keys; {failed} failed.',
    key: 'Key',
    user: 'User',
    reusable: 'Reusable',
    ephemeral: 'Ephemeral',
    expiration: 'Expiration',
    state: 'State',
    active: 'Active',
    used: 'Used',
    expired: 'Expired',
    savedConfirm: 'I have saved this key',
    plaintextWarning: 'This is the only time the full key is shown.',
    forceClose: 'Close without saving',
    confirmDelete: 'Type the key ID to confirm deletion',
  },
  authRequests: {
    registerTitle: 'Register new node',
    reauthTitle: 'Approve node re-authentication',
    registerDescription: 'A new node is waiting to be registered under a Headscale user.',
    reauthDescription: 'An existing node is waiting for re-authentication approval.',
    safetyWarning:
      'Headscale cannot show node details before approval. Only approve a request you initiated or trust.',
    requestId: 'Request ID',
    targetUser: 'Target user',
    register: 'Approve and register',
    approve: 'Approve re-authentication',
    reject: 'Reject request',
    confirmRegisterTitle: 'Confirm node registration',
    confirmRegisterMessage: 'Register this node under {user}?',
    confirmRegister: 'Confirm registration',
    confirmApproveTitle: 'Confirm approval',
    confirmApproveMessage: 'Approve this node re-authentication request?',
    confirmApprove: 'Confirm approval',
    confirmRejectTitle: 'Reject authentication request',
    confirmRejectMessage: 'Reject this authentication request?',
    confirmReject: 'Confirm rejection',
    results: {
      registered: 'Node registered successfully.',
      approved: 'Authentication approved. Return to the Tailscale client and wait for completion.',
      rejected: 'Authentication request rejected.',
    },
    viewNodes: 'View nodes',
    returnHome: 'Return to dashboard',
    errors: {
      invalid: 'This authentication request is invalid.',
      stale: 'This authentication request has expired or was already handled.',
      unauthorized: 'The API key was rejected or lacks permission.',
      timeout: 'The request timed out. Try again.',
      network: 'Could not reach Headscale. Try again.',
      unknown: 'The authentication request failed.',
    },
  },
  dashboard: {
    title: 'Dashboard',
    description: 'Monitor the health and activity of your Headscale network.',
    networkOverview: 'Network overview',
    networkOverviewHint: 'Current control-plane and route status.',
    needsAttention: 'Needs attention',
    needsAttentionHint: 'Resources that may require an administrator.',
    attentionRequired: 'Review required',
    allClear: 'All clear',
    allClearHint: 'No resources currently require attention.',
    users: 'Users',
    nodes: 'Nodes',
    onlineNodes: 'Online nodes',
    offlineNodes: 'Offline nodes',
    advertisedRoutes: 'Advertised routes',
    approvedRoutes: 'Approved routes',
    activeKeys: 'Active PreAuth keys',
    expiringKeys: 'Expiring PreAuth keys',
    offlineList: 'Recently offline nodes',
    expiringNodes: 'Expiring nodes',
    expiringKeyList: 'Expiring PreAuth keys',
    unapprovedRoutes: 'Advertised but not approved',
  },
  settings: {
    title: 'Settings',
    description: 'Configure the Headscale connection, refresh behavior, and console appearance.',
    connectionSection: 'Connection',
    connectionHint: 'Update the server address or replace the API key used by this browser.',
    apiKeyReplacement: 'Leave blank to keep the current key',
    connectionVerified: 'Connection verified',
    refreshSection: 'Refresh',
    refreshHint: 'Control how often live administration data is updated.',
    polling: 'Automatic polling',
    pollingHint: 'Keep tables and status indicators up to date.',
    appearanceSection: 'Appearance',
    appearanceHint: 'Choose the language, theme, and date presentation.',
    dangerSection: 'Disconnect',
    disconnectHint: 'Remove the stored credential and return to the connection screen.',
    disconnectMessage: 'Disconnect this console and remove the stored API key?',
    interval: 'Polling interval',
    dateStyle: 'Date and time',
    absolute: 'Absolute',
    relative: 'Relative',
    test: 'Test connection',
    apiKeyStatus: {
      title: 'API key status',
      hint: 'Review the expiration of the credential used by this browser.',
      loading: 'Loading API key status...',
      unavailable: 'Status unavailable',
      prefix: 'Prefix',
      expiration: 'Expiration',
      state: 'Status',
      states: {
        healthy: 'Healthy',
        'expiring-soon': 'Expiring soon',
        expired: 'Expired',
        'no-expiration': 'No expiration',
      },
      replace: 'Replace API key',
      replaceHint:
        'Replace the key manually so the browser never receives permission to create or revoke credentials.',
      steps: {
        create: 'Create a new 90-day key on the Headscale server.',
        test: 'Paste it above and test the connection.',
        save: 'Save after the new key is verified.',
        expire: 'Revoke the old key only after the replacement works.',
      },
      oneTimeWarning:
        'Headscale displays a newly created API key only once. Save it before leaving the terminal.',
    },
  },
}
