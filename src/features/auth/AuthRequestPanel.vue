...
import { maskAuthId } from '@/domain/auth-id'
...
const emit = defineEmits<{
  complete: [result: 'registered' | 'approved' | 'rejected']
  finish: []
}>()
const { t } = useI18n()
...
async function runPrimary() {
  ...
  try {
    if (isRegister.value) {
      ...
      emit('complete', 'registered')
    } else {
      await approve.mutateAsync(props.authId)
...
async function runReject() {
  ...
  errorMessage.value = null
  try {
    await reject.mutateAsync(props.authId)
    emit('complete', 'rejected')
...
<template>
  <div class="auth-request-panel">
    <template v-if="result">
      ...
      <NSpace justify="end">
        <NButton type="primary" @click="emit('finish')">
          {{ result === 'registered' ? t('authRequests.viewNodes') : t('authRequests.returnHome') }}
        </NButton>
      </NSpace>
    </template>
